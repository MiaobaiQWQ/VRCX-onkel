...
export const useGameLogStore = defineStore('GameLog', () => {
    ...
    const sessionsAllCollapsed = ref(false);
    const sessionsCollapseAllTrigger = ref(0);
    const sessionsExpandAllTrigger = ref(0);
    ...
    async function init() {
        ...
        sessionsAllCollapsed.value = await configRepository.getBool(
            'VRCX_gameLogSessionsAllCollapsed',
            false
        );
    }
     ...
     */
    async function setAllSessionsCollapsed(collapsed) {
        sessionsAllCollapsed.value = collapsed;
        await configRepository.setBool(
            'VRCX_gameLogSessionsAllCollapsed',
            collapsed
        );
        if (collapsed) {
            sessionsCollapseAllTrigger.value++;
        } else {
            sessionsExpandAllTrigger.value++;
        }
    }
    ...
    return {
        ...
        sessionsHasMore,
        sessionsVipFilter,
        sessionsAllCollapsed,
        sessionsCollapseAllTrigger,
        sessionsExpandAllTrigger,
