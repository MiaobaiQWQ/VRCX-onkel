...
export const useUpdateLoopStore = defineStore('UpdateLoop', () => {
    ...
    const state = {
        ...
        nextTrackedNonFriendsRefresh: 0
    };
    ...
    watch(
        ...
        () => {
            ...
            state.nextTrackedNonFriendsRefresh = 0;
        },
        { flush: 'sync' }
    );
    ...
    async function updateLoop() {
        try {
            if (watchState.isLoggedIn) {
                ...
                if (--state.nextTrackedNonFriendsRefresh <= 0) {
                    state.nextTrackedNonFriendsRefresh = 3600; // 1hour
                    refreshTrackedNonFriendsFlow().catch(console.error);
                }
            }
        } catch (err) {
