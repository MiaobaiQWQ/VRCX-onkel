    ...
    <div
        ...
        <template v-if="friend.ref">
            ...
            <div class="flex-1 overflow-hidden h-9 flex flex-col justify-between">
                ...
                <div v-if="isFriendActiveOrOffline" class="block truncate text-xs flex items-center gap-1">
                    <span class="truncate">{{ friend.ref.statusDescription }}</span>
                    <Timer
                        v-if="friend.ref.status && friend.ref.$status_at"
                        class="shrink-0 opacity-60 scale-90"
                        :epoch="friend.ref.$status_at" />
                </div>
                <template v-else>
