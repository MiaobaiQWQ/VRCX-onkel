            ...
            <div class="flex items-center w-full">
                <div class="options-container flex items-center gap-3 bg-transparent pb-3 shadow-none">
                    ...
                    <Input
                        ...
                        v-model="searchQuery"
                        class="h-9 w-48"
                        :placeholder="t('view.charts.mutual_friend.actions.search_highlight')"
                    />
                </div>
...
<script setup>
    ...
    function navigateToFriend(friendId) {
        ...
        highlightedNodeId.value = friendId;
    ...
    async function buildGraphFromMutualMap(mutualMap, meta = null) {
        ...
        nodeIds.forEach((id) => {
            ...
            const fullName = nodeNames.get(id) || id;
            const label = truncateLabelText(fullName);
    ...
    function renderGraph(graph, forceRecreate = false) {
        ...
        sigmaInstance.setSetting('nodeReducer', (node, data) => {
            ...
            const activeNode = hovered || highlightedNodeId.value;
            ...
            if (matchesSearch) {
                ...
                res.labelColor = '#111827';
                res.zIndex = 4; // Higher z-index for search matches
                return res;
            }
        ...
        sigmaInstance.setSetting('edgeReducer', (edge, data) => {
            ...
            const active = extremities[0] === activeNode || extremities[1] === activeNode;
        ...
        sigmaInstance.on('leaveNode', () => {
            hovered = null;
            rebuildNeighbors(highlightedNodeId.value);
            sigmaInstance.setSetting('labelRenderedSizeThreshold', highlightedNodeId.value ? 0 : DEFAULT_LABEL_THRESHOLD);
            sigmaInstance.refresh();
        });

        sigmaInstance.on('clickNode', ({ node }) => {
            if (node) {
                // Safety: Ensure camera is re-enabled if it was disabled by downNode
                if (isDragging) {
                    isDragging = false;
                    draggedNode = null;
                    sigmaInstance.getCamera().enable();
                }
                highlightedNodeId.value = node;
                rebuildNeighbors(node);
                sigmaInstance.refresh();
                showUserDialog(node);
            }
        ...
        sigmaInstance.on('mousemovebody', (e) => {
        ...
        sigmaInstance.on('mouseup', handleGlobalMouseUp);
