            ...
            <div
                ...
                <div class="ml-2" style="display: flex; flex-direction: column; min-width: 320px; width: 100%">
                    ...
                    <div class="mt-1.5">
                        ...
                        <Button
                            v-if="lastLocation.playerList.size > 0"
                            variant="outline"
                            size="sm"
                            class="ml-3 h-7 text-xs"
                            @click="trackAllNonFriends">
                            <Crosshair class="mr-1.5 h-3.5 w-3.5" />
                            {{ t('dialog.user.actions.tracked_nonfriend_add_all_in_instance') }}
                        </Button>
                        <div v-if="lastLocation.playerList.size > 0" class="ml-4 flex items-center gap-2">
                            <Switch id="auto-track" v-model="autoTrackNewUsers" size="sm" class="scale-90" />
                            <Label for="auto-track" class="text-xs text-muted-foreground cursor-pointer">{{ t('view.tools.other.auto_track_new_users') || '自动追踪新进来的用户' }}</Label>
                        </div>
                        <div v-if="lastLocation.playerList.size > 0" class="ml-4 flex items-center gap-2">
                            <Switch id="auto-track-join" v-model="autoTrackOnWorldJoin" size="sm" class="scale-90" />
                            <Label for="auto-track-join" class="text-xs text-muted-foreground cursor-pointer">{{ t('view.tools.other.auto_track_on_world_join') || '进入世界自动追踪所有人' }}</Label>
                        </div>
                    </div>
                    <div class="mt-1.5">
                        <span
...
<script setup>
    ...
    import { Apple, Crosshair, Home, Image, Monitor, Smartphone } from 'lucide-vue-next';
    ...
    watch(
        ...
        () => {
            ...
            if (autoTrackOnWorldJoin.value) {
                // Wait a bit for the player list to populate
                setTimeout(() => {
                    trackAllNonFriends();
                }, 2000);
            }
    ...
    watch(
        ...
        (newUsers, oldUsers) => {
            if (!autoTrackNewUsers.value) return;
            
            // Find users that are in newUsers but not in oldUsers
            const oldUserIds = new Set(oldUsers?.map(u => u.ref?.id).filter(Boolean) || []);
            const newJoiners = newUsers.filter(u => u.ref?.id && !oldUserIds.has(u.ref.id));

            newJoiners.forEach(user => {
                if (!user.isFriend && user.ref.id !== currentUser.value.id && !isTracked(user.ref.id)) {
                    const location = lastLocation.value?.location || '';
                    addTrackedNonFriend(user.ref.id, user.ref.displayName, typeof location === 'string' ? location : '', user.ref.bio || '');
                }
            });
        },
        { deep: true }
    ...
    const { autoTrackNewUsers, autoTrackOnWorldJoin } = storeToRefs(trackedNonFriendsStore);
    const { addTrackedNonFriend, isTracked } = trackedNonFriendsStore;
     ...
     *
     */
    async function trackAllNonFriends() {
        const nonFriends = currentInstanceUsersData.value.filter((user) => !user.isFriend && user.ref?.id && user.ref.id !== currentUser.value.id && !isTracked(user.ref.id));
        if (nonFriends.length === 0) {
            toast.info(t('message.common.no_new_targets') || '没有发现新的追踪目标');
            return;
        }
        ...
        for (const user of nonFriends) {
            const location = lastLocation.value?.location || '';
            await addTrackedNonFriend(user.ref.id, user.ref.displayName, typeof location === 'string' ? location : '', user.ref.bio || '');
            addedCount++;
        }

        toast.success(t('message.common.bulk_add_success', { count: addedCount }) || `已成功追踪 ${addedCount} 名玩家`);
    }
