...
const feed = {
    ...
    async searchFeedDatabase(
        ...
        if (vipList.length > 0) {
            ...
            // If no specific VIP list, exclude tracked non-friends from the general feed
            vipQuery = `AND user_id NOT IN (SELECT user_id FROM ${dbVars.userPrefix}_tracked_nonfriends)`;
        }
        let dateQuery = '';
        if (dateFrom) {
        ...
        await sqliteService.execute(
            (dbRow) => {
                results.push({
                    ...
                    created_at: dbRow[1],
                    bio: dbRow[2],
                    previous_bio: dbRow[3]
                });
            },
            `SELECT id, created_at, bio, previous_bio FROM ${dbVars.userPrefix}_feed_bio WHERE user_id = @userId ORDER BY created_at DESC`,
            { '@userId': userId }
        );
        return results;
    },
    ...
    async getLastStatusChangeForUser(userId) {
        ...
        }, `SELECT status, status_description, created_at FROM ${dbVars.userPrefix}_feed_status WHERE user_id = @userId ORDER BY created_at DESC LIMIT 1`, { '@userId': userId });
        return lastStatus;
    },
    ...
    async getLastBioChangeForUser(userId) {
        ...
        }, `SELECT bio, created_at FROM ${dbVars.userPrefix}_feed_bio WHERE user_id = @userId ORDER BY created_at DESC LIMIT 1`, { '@userId': userId });
        return lastBio;
    },

    async getLastGPSChangeForUser(userId) {
        let lastGps = null;
        await sqliteService.execute((dbRow) => {
            lastGps = {
                location: dbRow[0],
        ...
        }, `SELECT location, world_name, created_at FROM ${dbVars.userPrefix}_feed_gps WHERE user_id = @userId ORDER BY created_at DESC LIMIT 1`, { '@userId': userId });
        return lastGps;
    },

    async getLastOnlineOfflineForUser(userId) {
        let lastOnlineOffline = null;
        await sqliteService.execute((dbRow) => {
            lastOnlineOffline = {
                type: dbRow[0],
                created_at: dbRow[1]
            };
        }, `SELECT type, created_at FROM ${dbVars.userPrefix}_feed_online_offline WHERE user_id = @userId ORDER BY created_at DESC LIMIT 1`, { '@userId': userId });
        return lastOnlineOffline;
    },
    ...
    async getCoInstanceHistoryFromFeed(userIdA, userIdB) {
        const results = [];
        ...
        await sqliteService.execute(
            ...
            `SELECT
                a.previous_location as location,
                a.created_at AS friendALeave,
                a.time AS friendATime,
                b.created_at AS friendBLeave,
                b.time AS friendBTime
            FROM ${dbVars.userPrefix}_feed_gps a
            JOIN ${dbVars.userPrefix}_feed_gps b ON a.previous_location = b.previous_location
            WHERE a.user_id = @userIdA AND b.user_id = @userIdB
              AND a.previous_location NOT IN ('', 'offline', 'private', 'traveling')
