...
const gameLog = {
    ...
    /**
     * Get co-instance history between two friends
     * @param {string} userIdA
     * @param {string} userIdB
     * @returns {Promise<Array>}
     */
    async getCoInstanceHistoryBetweenFriends(userIdA, userIdB) {
        const results = [];
        await sqliteService.execute(
            (dbRow) => {
                results.push({
                    location: dbRow[0],
                    friendALeave: dbRow[1],
                    friendATime: dbRow[2] || 0,
                    friendBLeave: dbRow[3],
                    friendBTime: dbRow[4] || 0
                });
            },
            `SELECT
                a.location,
                a.created_at AS friendALeave,
                a.time AS friendATime,
                b.created_at AS friendBLeave,
                b.time AS friendBTime
            FROM gamelog_join_leave a
            JOIN gamelog_join_leave b ON a.location = b.location
            WHERE a.user_id = @userIdA AND a.type = 'OnPlayerLeft'
              AND b.user_id = @userIdB AND b.type = 'OnPlayerLeft'
    ...
    /**
     * Get current user's presence for a list of locations
     * @param {string} selfId
     * @param {string[]} locations
     * @returns {Promise<Map<string, Array>>}
     */
    async getSelfPresenceForLocations(selfId, locations) {
        const selfPresenceMap = new Map();
        if (!locations || locations.length === 0) return selfPresenceMap;

        const placeholders = [];
        const args = { '@selfId': selfId };
        ...
        await sqliteService.execute(
            (dbRow) => {
                ...
                if (!selfPresenceMap.has(loc)) {
                    selfPresenceMap.set(loc, []);
                }
                selfPresenceMap.get(loc).push(session);
            },
            `SELECT location, created_at, time
             FROM gamelog_join_leave
             WHERE user_id = @selfId AND type = 'OnPlayerLeft'
               AND location IN (${placeholders.join(', ')})`,
            args
        );
        return selfPresenceMap;
    },

    /**
     * Get maximum player count for a list of locations
     * @param {string[]} locations
     * @returns {Promise<Map<string, number>>}
     */
    async getMaxPlayerCountForLocations(locations) {
        const maxPlayerCountMap = new Map();
        if (!locations || locations.length === 0) return maxPlayerCountMap;
        ...
        await sqliteService.execute(
            (dbRow) => {
                maxPlayerCountMap.set(dbRow[0], dbRow[1]);
            },
        ...
        return maxPlayerCountMap;
    },

    /**
     * Get candidate co-instances for relationship suggestions
     * @param {string} selfId
     * @returns {Promise<object>}
     */
    async getCandidateCoInstances(selfId) {
        ...
        await sqliteService.execute(
            (dbRow) => {
                const loc = dbRow[0];
                const sess = { leaveAt: new Date(dbRow[1]).getTime(), time: dbRow[2] || 0 };
                if (!mySessions.has(loc)) mySessions.set(loc, []);
                mySessions.get(loc).push(sess);
            },
            `SELECT location, created_at, time FROM gamelog_join_leave WHERE user_id = @selfId AND type = 'OnPlayerLeft'`,
            { '@selfId': selfId }
        );

        // 2. Get all player sessions in those locations
        const myLocations = Array.from(mySessions.keys());
        if (myLocations.length > 0) {
            const placeholders = [];
            const args = { '@selfId': selfId };
            myLocations.forEach((loc, i) => {
                const key = `@loc_${i}`;
                args[key] = loc;
                placeholders.push(key);
            });
            ...
            await sqliteService.execute(
                (dbRow) => {
                    const loc = dbRow[0];
                    const userId = dbRow[1];
                    const leaveAt = new Date(dbRow[2]).getTime();
                    const time = dbRow[3] || 0;
                    
                    if (userId === selfId) return;

                    if (!eventsByLocation.has(loc)) eventsByLocation.set(loc, []);
                    eventsByLocation.get(loc).push({ userId, leaveAt, time });

                    // Update first/last seen
                    const startAt = leaveAt - time;
                    if (!firstSeen.has(userId) || startAt < firstSeen.get(userId)) firstSeen.set(userId, startAt);
                    if (!lastSeen.has(userId) || leaveAt > lastSeen.get(userId)) lastSeen.set(userId, leaveAt);
                },
                `SELECT location, user_id, created_at, time 
                 FROM gamelog_join_leave 
                 WHERE type = 'OnPlayerLeft' AND user_id != '' AND user_id != @selfId
                 AND location IN (${placeholders.join(', ')})
        ...
        const oldMutualSnapshot = await mutualGraph.getMutualGraphSnapshot();
    ...
    /**
     * Get relationship timeline data for the charts.
     * Aggregates total shared time and join counts per day per user.
     * @returns {Promise<Array<{day: string, userId: string, displayName: string, totalTime: number, joinCount: number}>>}
     */
    async getRelationshipTimelineData() {
        const results = [];
        if (!dbVars.userId) return results;
        ...
        await sqliteService.execute(
            ...
            `SELECT
                ...
                user_id,
                MAX(display_name),
                SUM(CASE WHEN type = 'OnPlayerLeft' THEN time ELSE 0 END) AS total_time,
                SUM(CASE WHEN type = 'OnPlayerJoined' THEN 1 ELSE 0 END) AS join_count
            FROM gamelog_join_leave
            WHERE user_id IS NOT NULL 
              AND user_id != '' 
              AND user_id != @selfUserId
            GROUP BY day, user_id
            HAVING total_time > 0 OR join_count > 0
            ORDER BY day ASC`,
            { '@selfUserId': dbVars.userId }
        );
        return results;
    }
};
