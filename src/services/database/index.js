...
function getTableName(name) {
    return dbVars.instancePrefix + name;
}
...
const database = {
    ...
    async initUserTables(userId) {
        ...
        // Add instance2 prefix if applicable
        if (typeof AppApi !== 'undefined' && AppApi.GetIsInstance2()) {
            dbVars.instancePrefix = 'instance2_';
        }
        ...
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${dbVars.userPrefix}_tracked_nonfriends (user_id TEXT PRIMARY KEY, display_name TEXT, added_at TEXT, location TEXT)`
        );
        try {
            await sqliteService.executeNonQuery(`ALTER TABLE ${dbVars.userPrefix}_tracked_nonfriends ADD COLUMN location TEXT`);
        } catch (e) {
            // Column already exists
        }
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${dbVars.userPrefix}_manual_relations (user_id_a TEXT, user_id_b TEXT, relation_type TEXT, added_at TEXT, PRIMARY KEY (user_id_a, user_id_b))`
        );
        await sqliteService.executeNonQuery(
    ...
    async syncFromMainInstance() {
        if (!dbVars.instancePrefix || !dbVars.userPrefix) return;
        const mainPrefix = dbVars.userPrefix.replace(dbVars.instancePrefix, '');
        
        // Ensure the main database is attached
        if (typeof AppApi !== 'undefined' && AppApi.GetAppDataDirectory) {
            const mainAppData = await AppApi.GetAppDataDirectory(false);
            if (mainAppData) {
                const mainDbPath = mainAppData + '\\VRCX.sqlite3';
                try {
                    await sqliteService.executeNonQuery(`ATTACH DATABASE '${mainDbPath}' AS main_db`);
                } catch (e) {
                    console.error('Failed to attach main database', e);
                }
            }
        }
        ...
        for (const table of tablesToSync) {
            const sourceTable = `main_db.${mainPrefix}_${table}`;
            const targetTable = `${dbVars.userPrefix}_${table}`;
            
            // Check if source table exists
            let sourceExists = false;
            await sqliteService.execute(() => { sourceExists = true; }, `SELECT name FROM main_db.sqlite_master WHERE type='table' AND name='${mainPrefix}_${table}'`);
            
            if (sourceExists) {
                await sqliteService.executeNonQuery(
                    `INSERT OR IGNORE INTO ${targetTable} SELECT * FROM ${sourceTable}`
                );
            }
        }
    },
