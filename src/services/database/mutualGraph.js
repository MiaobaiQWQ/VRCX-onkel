...
const mutualGraph = {
    async getMutualGraphSnapshot() {
        ...
        try {
            ...
            console.error('Failed to read from mutual graph table, maybe readOnlySync database not attached properly', e);
        }
        return snapshot;
    },
    ...
    // Aliases for compatibility with older code or other modules
    async getMutualGraphSnapshotFromOld() {
        return this.getMutualGraphSnapshot();
    },

    async mergeMutualLinksToOld(entries) {
        // No-op or implement if needed for legacy sync
    },
