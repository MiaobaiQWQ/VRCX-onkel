...
export function useUserDialogCommands(
    ...
    function buildCommandMap() {
        ...
        return {
            ...
            'Toggle Tracking': () => {
                const { toggleTrackedNonFriend } = useTrackedNonFriendsStore();
                const locationStore = useLocationStore();
                const location = locationStore.lastLocation?.location || locationStore.lastLocation?.value?.location || '';
                toggleTrackedNonFriend(D().id, D().ref.displayName, typeof location === 'string' ? location : '', D().ref.bio || '');
            },
