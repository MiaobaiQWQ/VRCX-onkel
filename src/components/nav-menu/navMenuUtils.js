...
export function sanitizeLayout(
    ...
    const chartsKeys = [
        ...
        'charts-mutual',
        'charts-hot-worlds',
        'charts-two-person-relationship',
        'charts-timeline'
    ];
    ...
    const appendSocialFolder = (target = normalized) => {
        // Just a helper to add the folder if entirely missing,
        // though social folder should exist for most users.
        const socialKeys = ['friend-log', 'friend-list', 'moderation', 'tool-tracked-non-friends'];
        ...
        target.push({
            ...
            icon: 'ri-group-line',
            items: socialKeys.filter((k) => definitionMap.has(k))
        });
    };
    ...
    // Force append `tool-tracked-non-friends` to social folder if it's not anywhere and not hidden
    if (!usedKeys.has('tool-tracked-non-friends') && !hiddenSet.has('tool-tracked-non-friends') && definitionMap.has('tool-tracked-non-friends')) {
        const socialFolder = normalized.find(
            (entry) => entry.type === 'folder' && entry.id === 'default-folder-social'
        );
        if (socialFolder && Array.isArray(socialFolder.items)) {
            socialFolder.items.push('tool-tracked-non-friends');
            usedKeys.add('tool-tracked-non-friends');
        }
    }
