...
## 🚀 双开功能 (Dual-Instance Feature)
VRCX 现已原生支持双开模式，允许你在本地同时运行两个完全独立的 VRCX 进程，数据互不干扰。

### 启动方式
你可以通过以下方式启动第二实例：
1. **界面内启动**：在主实例的 `设置 -> 双开管理` 中点击“启动第二实例”按钮。
2. **命令行参数**：通过命令行或快捷方式追加 `--instance2` 参数启动。
   - 可选参数：
     - `--user-data-dir="C:\Path\To\Data"` 指定自定义数据目录。
     - `--profile-directory="ProfileName"` 指定 Chrome 配置文件名。
     - `--port=34583` 指定 Overlay IPC 端口。
     - `--js-flags="--max-old-space-size=512"` 限制内存。

### 功能亮点
- **数据完全隔离**：第二实例拥有独立的数据库（位于 `VRCX_instance2` 目录）、独立的缓存和日志文件。
- **端口冲突自动解决**：当检测到 Overlay IPC 端口被占用时，会自动递增端口以避免冲突。
- **托盘图标区分**：第二实例会在系统托盘和任务栏使用带角标/异色的图标，方便区分。
- **增强模块（Jirai）集成**：第二实例可独立启用增强模块，包括：
  - 双人关系查询
  - 非好友追踪
  - 灯色分布图（在线活跃状态饼图）
  - 好友关系网增强
- **主实例只读同步**：主实例可通过设置开启“只读同步”，直接在界面中查看第二实例的关系网和追踪数据，无需来回切换。
