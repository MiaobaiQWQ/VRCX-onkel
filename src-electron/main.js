...
const profileDir = getArgValue('--profile-directory');
...
function createTray() {
    ...
    if (process.platform === 'darwin') {
        ...
        if (image.isEmpty() && isInstance2) {
            trayIcon = nativeImage.createFromPath(path.join(rootDir, 'images/VRCX.png')).resize({ width: 64, height: 64 });
        } else {
        ...
        const icoPath = path.join(rootDir, `images/VRCX${iconSuffix}.ico`);
        const notifyIcoPath = path.join(rootDir, `images/VRCX_notify${iconSuffix}.ico`);
        
        trayIcon = fs.existsSync(icoPath) ? icoPath : path.join(rootDir, 'images/VRCX.ico');
...
function getElectronUserDataPath() {
    ...
    if (process.platform === 'win32') {
        return path.join(vrcxPath, electronUserData);
    }
    if (process.platform === 'darwin') {
        if (userDataDir) {
            return path.join(vrcxPath, electronUserData);
...
function getVRCXPath() {
    if (userDataDir) {
        return path.join(userDataDir, profileDir || 'Default');
    }
    if (process.platform === 'win32') {
