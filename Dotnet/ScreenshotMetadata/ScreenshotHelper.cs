            ...
            foreach (var file in files)
            {
                ...
                switch (searchType)
                {
                    case ScreenshotSearchType.Username:
                        if (string.IsNullOrEmpty(query) || metadata.ContainsPlayerName(query, true, true))
                            result.Add(metadata);

                        break;
                    case ScreenshotSearchType.UserID:
                        if (string.IsNullOrEmpty(query) || metadata.ContainsPlayerID(query))
                            result.Add(metadata);

                        break;
                    case ScreenshotSearchType.WorldName:
                        if (string.IsNullOrEmpty(query) || (metadata.World.Name != null && metadata.World.Name.Contains(query, StringComparison.OrdinalIgnoreCase)))
                            result.Add(metadata);

                        break;
                    case ScreenshotSearchType.WorldID:
                        if (string.IsNullOrEmpty(query) || metadata.World.Id == query)
                            result.Add(metadata);
