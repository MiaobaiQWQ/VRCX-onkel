    ...
    public partial class MainForm : WinformBase
    {
        ...
        public MainForm()
        {
            ...
            try
            {
                ...
                var iconSuffix = StartupArgs.LaunchArguments.IsInstance2 ? "2" : "";
                
                var normalIconPath = Path.Combine(path, $"VRCX{iconSuffix}.ico");
                if (!File.Exists(normalIconPath)) normalIconPath = Path.Combine(path, "VRCX.ico");
                
                var notifyIconPath = Path.Combine(path, $"VRCX_notify{iconSuffix}.ico");
                if (!File.Exists(notifyIconPath)) notifyIconPath = Path.Combine(path, "VRCX_notify.ico");
                
                _appIcon = new Icon(normalIconPath);
                _appIconNoty = new Icon(notifyIconPath);
                Icon = _appIcon;
                TrayIcon.Icon = _appIcon;
                if (StartupArgs.LaunchArguments.IsInstance2)
                {
                    TrayIcon.Text = "VRCX (Instance 2)";
                }
            }
            catch (Exception ex)
            {
