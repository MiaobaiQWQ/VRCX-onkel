    ...
    public static class Program
    {
        ...
        private static void SetProgramDirectories()
        {
            if (string.IsNullOrEmpty(AppDataDirectory))
            {
                ...
                {
                    var profile = StartupArgs.LaunchArguments.ProfileDirectory ?? "Default";
                    AppDataDirectory = Path.Join(StartupArgs.LaunchArguments.UserDataDir, profile);
                }
                else
                {
        ...
        private static void Run()
        {
            ...
            if (StartupArgs.LaunchArguments.Port > 0)
            {
                OverlayServer.Port = StartupArgs.LaunchArguments.Port;
            }
            else if (StartupArgs.LaunchArguments.IsInstance2)
            {
                // Default port for instance 2 if not specified
                OverlayServer.Port = 34583; 
            }
