...
namespace VRCX
{
    public partial class AppApiElectron
    {
        ...
        public override bool OpenVrcxCrashDumps()
        {
            ...
            var electronUserData = isInstance2 ? "ElectronUserData2" : "ElectronUserData";
            var path = Path.Join(Program.AppDataDirectory, electronUserData, "CrashDumps");
        ...
        {
            var path = Path.Join(Program.AppDataDirectory, "Moss");
            if (!Directory.Exists(path))
            {
                try
                {
