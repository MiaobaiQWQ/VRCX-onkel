...
namespace VRCX
{
    public partial class AppApiCef
    {
        ...
        public override bool OpenVrcxCrashDumps()
        {
            var isInstance2 = StartupArgs.LaunchArguments.IsInstance2;
            var path = Path.Join(Program.AppDataDirectory, isInstance2 ? "instance2/userdata/CrashDumps" : "userdata/CrashDumps");
            if (!Directory.Exists(path))
            {
                try
                {
        ...
        public override bool OpenMossFolder()
        {
            var path = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "VRCX", "Moss");
            if (!Directory.Exists(path))
            {
                try
                {
        ...
        public override bool OpenInstance2AppDataFolder()
        {
            // By default, instance 2 uses a different folder if --user-data-dir is not specified, 
            // but here we just open the default one if it exists or create it.
            // In StartupArgs.cs, Program.AppDataDirectory might be changed.
            var path = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "VRCX", "Instance2");
            if (!Directory.Exists(path))
            {
                try
                {
